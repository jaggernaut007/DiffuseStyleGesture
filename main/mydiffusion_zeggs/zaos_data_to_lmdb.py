...
from process_zaos_bvh import preprocess_animation, pose2bvh
...
def make_zaos_dataset(source_path, target):
    if not os.path.exists(target):
        os.mkdir(target)

    def make_zaos_subdataset(source_path, target, all_poses):
        if not os.path.exists(target):
            os.mkdir(target)
    ...
    all_poses = []
    all_poses = make_zaos_subdataset(source_path_train, target_train, all_poses)
    source_path_test = os.path.join(source_path, 'valid')
    target_test = os.path.join(target, 'valid')
    all_poses = make_zaos_subdataset(source_path_test, target_test, all_poses)
...
if __name__ == '__main__':
    '''
    python zaos_data_to_lmdb.py
    '''
    source_path = '/home/challenge-user/poc/DiffuseStyleGesture/main/mydiffusion_zeggs/datasets/zaos1'
    target = '/home/challenge-user/poc/DiffuseStyleGesture/main/mydiffusion_zeggs/datasets/processed_v2'
    make_zaos_dataset(source_path, target)
